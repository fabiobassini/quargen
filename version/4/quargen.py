...
def main():
    ...
    subparsers = parser.add_subparsers(dest="command", required=True,
                                       help="Scegli un comando tra: generate, dev, build, add")

    # Comando generate
    gen_epilog = (
        "Esempi di utilizzo:\n"
        "  python quargen.py generate --name MyModule --base /percorso/progetto\n"
        "Questo comando genera la struttura completa di un nuovo modulo in '/percorso/progetto/MyModule'."
    )
    gen_parser = subparsers.add_parser("generate",
                                       help="Genera la struttura di un nuovo modulo (include main.py, webpack.config.js e package.json).",
                                       epilog=gen_epilog,
                                       formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    dev_epilog = (
        ...
        "Questo comando avvia l'applicazione in modalità sviluppo eseguendo main.py nella directory specificata."
    )
    dev_parser = subparsers.add_parser("dev",
                                       help="Avvia l'applicazione in modalità sviluppo (usa main.py).",
                                       epilog=dev_epilog,
                                       formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    build_epilog = (
        ...
        "Questo comando esegue la build in modalità produzione tramite webpack nella directory specificata."
    )
    build_parser = subparsers.add_parser("build",
                                         help="Esegue la build in modalità produzione tramite webpack (JS minimizzato).",
                                         epilog=build_epilog,
                                         formatter_class=argparse.RawDescriptionHelpFormatter)
    ...
    add_epilog = (
        ...
        "Questo comando aggiunge un nuovo template UI chiamato 'CustomTemplate' al modulo e genera anche un endpoint con URL prefix '/custom'.\n\n"
        "Altri esempi:\n"
        "  python quargen.py add --type controller --class_name NewController --module /percorso/progetto/MyModule\n"
        "  python quargen.py add --type service --class_name NewService --module /percorso/progetto/MyModule"
    )
    add_parser = subparsers.add_parser("add",
                                       help="Aggiunge una nuova classe o template al modulo.",
                                       epilog=add_epilog,
                                       formatter_class=argparse.RawDescriptionHelpFormatter)
